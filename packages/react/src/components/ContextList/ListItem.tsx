...
import { getElementType, getUnhandledProps, useAccessibility } from '@fluentui/react-bindings'
...
import { useListConsumer } from './ListContext'
...
type ListItemComponent = React.FC<WithAsProp<ListItemProps>> & {
  create: ShorthandFactory<ListItemProps>
  className: string
  slotClassNames: ListItemSlotClassNames
}

const ListItem: ListItemComponent = props => {
  const {
    content,
    contentMedia,
    header,
  ...
  const [classes, styles] = useStyles(ListItem.displayName, {
    className: ListItem.className,
    mapPropsToStyles: () => ({ ...parentProps, ...props }),
  })
